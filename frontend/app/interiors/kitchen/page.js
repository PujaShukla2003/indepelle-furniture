...
export const metadata = {
  title: "Kitchen Interiors | Indepelle",
  description:
    "Luxury modular and modern kitchen interior designs by Indepelle",
};

/* 🔹 Price / rating generator */
function getKitchenMeta(index) {
  const prices = [185000, 215000, 255000, 295000, 360000];
  const ratings = [4.5, 4.6, 4.7, 4.8, 4.9];
  const reviews = [52, 78, 104, 139, 191];
...
export default function KitchenPage() {
  const dirPath = path.join(
    process.cwd(),
    "public/images/interiors/kitchen"
  );
    ...
    <section className="max-w-7xl mx-auto px-6 py-16">
      ...
      {images.length === 0 ? (
        <p className="text-gray-500">
          No kitchen images available
        </p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
          {images.map((img, i) => {
            const meta = getKitchenMeta(i);
            ...
            return (
              ...
              >
                ...
                <div className="relative overflow-hidden bg-gray-100">
                  <Image
                    src={`/images/interiors/kitchen/${img}`}
