export const products = [
  // ================== SOFAS ==================
  {
    id: "1",
    category: "sofas",
    name: "Luxury Fabric Sofa",
    price: 124999,
    images: [
      "https://images.pexels.com/photos/1866149/pexels-photo-1866149.jpeg",
      "https://images.pexels.com/photos/1350789/pexels-photo-1350789.jpeg",
      "https://images.pexels.com/photos/271816/pexels-photo-271816.jpeg",
    ],
    rating: 4.6,
    reviews: 128,
    description:
      "Premium quality fabric sofa designed for superior comfort and modern living spaces.",
    highlights: [
      "Premium fabric upholstery",
      "High density foam cushions",
      "Strong wooden frame",
      "Perfect for modern homes",
    ],
  },

  {
    id: "2",
    category: "sofas",
    name: "Modern Wooden Sofa",
    price: 129999,
    images: [
      "https://images.unsplash.com/photo-1586023492125-27b2c045efd7",
      "https://images.unsplash.com/photo-1616627983427-9f7c8e77c7db",
      "https://images.unsplash.com/photo-1600585154340-be6161a56a0c",
    ],
    rating: 4.4,
    reviews: 94,
    description:
      "Elegant wooden sofa crafted with durability and timeless style.",
    highlights: [
      "Solid wood construction",
      "Long-lasting durability",
      "Minimal modern design",
      "Perfect for living rooms",
    ],
  },

  {
    id: "3",
    category: "sofas",
    name: "Comfort Recliner Sofa",
    price: 134999,
    images: [
      "https://images.pexels.com/photos/276528/pexels-photo-276528.jpeg",
      "https://images.pexels.com/photos/5998050/pexels-photo-5998050.jpeg",
      "https://images.pexels.com/photos/1866149/pexels-photo-1866149.jpeg",
    ],
    rating: 4.8,
    reviews: 162,
    description:
      "Relax in luxury with this premium comfort recliner sofa.",
    highlights: [
      "Smooth recliner mechanism",
      "Extra comfort cushioning",
      "Premium leatherette finish",
      "Ideal for relaxation",
    ],
  },

  // ================= BEDS =================
{
  id: "6",
  category: "bedroom",
  name: "Luxury King Size Bed",
  price: 245999,
  images: [
    "https://images.pexels.com/photos/1648776/pexels-photo-1648776.jpeg",
    "https://images.pexels.com/photos/271624/pexels-photo-271624.jpeg",
    "https://images.pexels.com/photos/1866149/pexels-photo-1866149.jpeg",
  ],
  rating: 4.7,
  reviews: 143,
  description: "Premium king size bed for ultimate comfort and luxury.",
  highlights: [
    "Solid wood frame",
    "Premium mattress support",
    "Elegant headboard design",
    "Long-lasting durability",
  ],
},

{
  id: "7",
  category: "bedroom",
  name: "Modern Queen Size Bed",
  price: 35999,
  images: [
    "https://images.pexels.com/photos/271624/pexels-photo-271624.jpeg",
    "https://images.pexels.com/photos/1648776/pexels-photo-1648776.jpeg",
    "https://images.pexels.com/photos/276528/pexels-photo-276528.jpeg",
  ],
  rating: 4.5,
  reviews: 98,
  description: "Stylish queen size bed for modern bedrooms.",
  highlights: [
    "Minimal modern look",
    "Strong base support",
    "Easy assembly",
    "Premium finish",
  ],
},

// ================= DINING =================
{
  id: "8",
  category: "dining",
  name: "6 Seater Dining Table",
  price: 138999,
  images: [
    "https://images.pexels.com/photos/276528/pexels-photo-276528.jpeg",
    "https://images.pexels.com/photos/584399/pexels-photo-584399.jpeg",
    "https://images.pexels.com/photos/271816/pexels-photo-271816.jpeg",
  ],
  rating: 4.6,
  reviews: 120,
  description: "Elegant 6 seater dining table for family meals.",
  highlights: [
    "Solid wood build",
    "Spacious tabletop",
    "Premium polish",
    "Perfect for families",
  ],
},

{
  id: "9",
  category: "dining",
  name: "4 Seater Dining Set",
  price: 168999,
  images: [
    "https://images.pexels.com/photos/584399/pexels-photo-584399.jpeg",
    "https://images.pexels.com/photos/276528/pexels-photo-276528.jpeg",
    "https://images.pexels.com/photos/1866149/pexels-photo-1866149.jpeg",
  ],
  rating: 4.4,
  reviews: 87,
  description: "Compact dining set for small families.",
  highlights: [
    "Space saving design",
    "Comfortable seating",
    "Strong wooden legs",
    "Modern style",
  ],
},

// ================= STORAGE =================
{
  id: "10",
  category: "storage",
  name: "Wooden Wardrobe",
  price: 149999,
  images: [
    "https://images.pexels.com/photos/271816/pexels-photo-271816.jpeg",
    "https://images.pexels.com/photos/1648776/pexels-photo-1648776.jpeg",
    "https://images.pexels.com/photos/276528/pexels-photo-276528.jpeg",
  ],
  rating: 4.8,
  reviews: 156,
  description: "Spacious wooden wardrobe with modern finish.",
  highlights: [
    "Large storage space",
    "Premium hinges",
    "Scratch resistant",
    "Elegant design",
  ],
},

{
  id: "11",
  category: "storage",
  name: "Modern Chest of Drawers",
  price: 131999,
  images: [
    "https://images.pexels.com/photos/1866149/pexels-photo-1866149.jpeg",
    "https://images.pexels.com/photos/271816/pexels-photo-271816.jpeg",
    "https://images.pexels.com/photos/276528/pexels-photo-276528.jpeg",
  ],
  rating: 4.5,
  reviews: 102,
  description: "Stylish chest of drawers for bedroom storage.",
  highlights: [
    "Smooth drawer system",
    "Compact design",
    "Premium wood finish",
    "Easy maintenance",
  ],
},


  // ================== LIVING ==================
  {
    id: "4",
    category: "living",
    name: "Modern Living Chair",
    price: 118999,
    images: [
      "https://images.pexels.com/photos/1571460/pexels-photo-1571460.jpeg",
      "https://images.pexels.com/photos/1866149/pexels-photo-1866149.jpeg",
      "https://images.pexels.com/photos/276528/pexels-photo-276528.jpeg",
    ],
    rating: 4.5,
    reviews: 76,
    description:
      "Stylish living chair designed to elevate modern interiors.",
    highlights: [
      "Comfortable cushioning",
      "Premium fabric finish",
      "Strong wooden legs",
      "Perfect for living rooms",
    ],
  },

  {
    id: "5",
    category: "living",
    name: "Luxury Living Sofa Set",
    price: 142999,
    images: [
      "https://images.pexels.com/photos/5998050/pexels-photo-5998050.jpeg",
      "https://images.pexels.com/photos/276528/pexels-photo-276528.jpeg",
      "https://images.pexels.com/photos/1866149/pexels-photo-1866149.jpeg",
    ],
    rating: 4.7,
    reviews: 112,
    description:
      "Premium living room sofa set with elegant design and superior comfort.",
    highlights: [
      "Luxury fabric upholstery",
      "Deep comfort seating",
      "Modern premium look",
      "Ideal for large living rooms",
    ],
  },

  {
    id: "6",
    category: "living",
    name: "Compact Living Armchair",
    price: 115999,
    images: [
      "https://images.pexels.com/photos/1350789/pexels-photo-1350789.jpeg",
      "https://images.pexels.com/photos/1571460/pexels-photo-1571460.jpeg",
      "https://images.pexels.com/photos/271816/pexels-photo-271816.jpeg",
    ],
    rating: 4.3,
    reviews: 58,
    description:
      "Compact and stylish armchair perfect for small living spaces.",
    highlights: [
      "Space-saving design",
      "Soft cushioned seat",
      "Durable frame",
      "Modern aesthetic",
    ],
  },
];
