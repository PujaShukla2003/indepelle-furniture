...
export const metadata = {
  title: "Rugs | Indepelle Living",
  description:
    "Premium living room rugs with elegant, modern and luxury designs",
};

/* 🔹 Price / rating auto generator */
function getRugMeta(index) {
  const prices = [5999, 7999, 9999, 12999, 15999];
  const ratings = [4.2, 4.4, 4.5, 4.6, 4.8];
  const reviews = [28, 49, 71, 96, 138];
    ...
    <section className="max-w-7xl mx-auto px-6 py-16">
      ...
      {images.length === 0 ? (
        <p className="text-gray-500">
          No rug images available
        </p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
          {images.map((img, i) => {
            const meta = getRugMeta(i);
